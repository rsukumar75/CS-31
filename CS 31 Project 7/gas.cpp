...
bool isOrthogonal(Flatulan* f, Player* p);
bool isDiagonal(Flatulan* f, Player* p);
...
void Flatulan::move()
{
    ...
    {
        case UP:
            if(m_row-1>0 && (m_city->player()->row()!=m_row-1 || m_col!=m_city->player()->col()))
                m_row--;
            break;
        case DOWN:
            if(m_row+1<=m_city->rows() && (m_city->player()->row()!=m_row+1 || m_col!=m_city->player()->col()))
                m_row++;
            break;
        case LEFT:
            if(m_col-1>0 && (m_city->player()->col()!=m_col-1 || m_row!=m_city->player()->row()))
                m_col--;
            break;
        case RIGHT:
            if(m_col+1<=m_city->cols() && (m_city->player()->col()!=m_col+1 || m_row!=m_city->player()->row()))
                m_col++;
            break;
    }
}
...
void Player::move(int dir)
{
    ...
    switch(dir)
    {
        case UP:
            ...
            break;
        case DOWN:
            if(m_row+1<=m_city->rows() && m_city->nFlatulansAt(m_row+1, m_col)==0)
                m_row++;
            break;
        case LEFT:
            ...
            break;
        case RIGHT:
            if(m_col+1<=m_city->cols() && m_city->nFlatulansAt(m_row, m_col+1)==0)
                m_col++;
            break;
    }
    ...
    for(r=0;r<rows();r++)
    {
        for(c=0;c<cols();c++)
        {
            if(nFlatulansAt(r+1, c+1)==1)
                grid[r][c]='F';
            else if(nFlatulansAt(r+1, c+1)>=2 && nFlatulansAt(r+1, c+1)<9)
                grid[r][c]='0'+nFlatulansAt(r+1, c+1);
...
void City::preachToFlatulansAroundPlayer()
{
    ...
    {
        if(isOrthogonal(m_flatulans[i],m_player) || isDiagonal(m_flatulans[i], m_player))
        {
...
void City::moveFlatulans()
{
    ...
    {
        m_flatulans[k]->move();
        if(isOrthogonal(m_flatulans[k], m_player))
            m_player->getGassed();
    }
}
...
bool isOrthogonal(Flatulan* f, Player* p)
{
    if((f->row()==p->row()-1 && f->col()==p->col()) || (f->row()==p->row()+1 && f->col()==p->col()) || (f->col()==p->col()-1 && f->row()==p->row()) || (f->col()==p->col()+1 && f->row()==p->row()))
        return true;
    return false;
}
...
bool isDiagonal(Flatulan* f, Player* p)
{
    if(((f->row()==p->row()+1) && ((f->col()==p->col()+1)||(f->col()==p->col()-1))) || ((f->row()==p->row()-1) && ((f->col()==p->col()+1)||(f->col()==p->col()-1))))
        return true;
    return false;
}
